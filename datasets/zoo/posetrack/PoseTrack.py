...
from datasets.process import get_affine_transform, fliplr_joints, exec_affine_transform, generate_heatmaps, half_body_transform, \
    convert_data_to_annorect_struct, generate_motion_heatmaps
...
class PoseTrack(VideoDataset):
    ...
    def _get_spatiotemporal_window(self, data_item):
        ...
        if self.distance_whole_otherwise_segment:
            ...
            next_delta_range = range(2, min((num_frames - current_idx) if is_posetrack18 else (
                        num_frames - current_idx + 1),
                                            farthest_distance))
        else:
        ...
        if self.train:
            if (next_delta < 2) or (prev_delta < 2):
            ...
            if optical_image_existed:
                ...
                optical_origin = origin_path.split('/')[-4:]
                optical_origin = '/'.join(optical_origin)

                prev_2_optical_path = osp.join(prev_2_optical_root_path, optical_origin)
                next_2_optical_path = osp.join(next_2_optical_root_path, optical_origin)
                prev_optical_path = osp.join(prev_optical_root_path, optical_origin)
                next_optical_path = osp.join(next_optical_root_path, optical_origin)

                prev_2_optical_image = read_image(prev_2_optical_path)
                next_2_optical_image = read_image(next_2_optical_path)
                prev_optical_image = read_image(prev_optical_path)
                next_optical_image = read_image(next_optical_path)
            else:
                prev_optical_image = None
                next_optical_image = None
                prev_2_optical_image = None
                next_2_optical_image = None
        ...
        if self.is_train:
            ...
            if random.random() <= 0.5:
                ...
                data_numpy_prev_2 = (T.GaussianBlur(kernel_size=(5, 9),
                                                   sigma=(0.1, 5))(
                    torch.from_numpy(data_numpy_prev_2.copy()))).numpy()
            ...
            if random.random() <= 0.3:
                ...
                data_numpy_next = torch.from_numpy(data_numpy_next.copy()).permute(2, 0, 1)
                data_numpy_prev_2 = torch.from_numpy(data_numpy_prev_2.copy()).permute(2, 0, 1)
                data_numpy_next_2 = torch.from_numpy(data_numpy_next_2.copy()).permute(2, 0, 1)

                data_numpy = (T.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5)(data_numpy)).numpy()
                data_numpy_prev = (T.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5)(data_numpy_prev)).numpy()
                data_numpy_next = (T.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5)(data_numpy_next)).numpy()
                data_numpy_prev_2 = (T.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5)(data_numpy_prev_2)).numpy()
                data_numpy_next_2 = (T.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5)(data_numpy_next_2)).numpy()

                data_numpy = data_numpy.transpose(1, 2, 0)
                data_numpy_prev = data_numpy_prev.transpose(1, 2, 0)
        ...
        if self.train:
            if prev_optical_image is None or next_optical_image is None or prev_2_optical_image is None or next_2_optical_image is None:
                pass
            else:
                prev_optical_image = cv2.warpAffine(prev_optical_image, trans, (int(self.image_size[0]), int(self.image_size[1])),
                                               flags=cv2.INTER_LINEAR)
                next_optical_image = cv2.warpAffine(next_optical_image, trans,
                                                    (int(self.image_size[0]), int(self.image_size[1])),
                                                    flags=cv2.INTER_LINEAR)
                prev_2_optical_image = cv2.warpAffine(prev_2_optical_image, trans,
                                                    (int(self.image_size[0]), int(self.image_size[1])),
                                                    flags=cv2.INTER_LINEAR)
                next_2_optical_image = cv2.warpAffine(next_2_optical_image, trans,
                                                    (int(self.image_size[0]), int(self.image_size[1])),
                                                    flags=cv2.INTER_LINEAR)
        ...
        if self.train:
            prev_target_motion_heatmaps, target_heatmaps_weight = generate_motion_heatmaps(joints, joints_vis, self.sigma, self.image_size,
                                                                        self.heatmap_size,
                                                                        self.num_joints, prev_optical_image,
                                                                        use_different_joints_weight=self.use_different_joints_weight,
                                                                        joints_weight=self.joints_weight)
            next_target_motion_heatmaps, target_heatmaps_weight = generate_motion_heatmaps(joints, joints_vis, self.sigma, self.image_size,
                                                                        self.heatmap_size,
                                                                        self.num_joints, next_optical_image,
                                                                        use_different_joints_weight=self.use_different_joints_weight,
                                                                        joints_weight=self.joints_weight)
            prev_2_target_motion_heatmaps, target_heatmaps_weight = generate_motion_heatmaps(joints, joints_vis, self.sigma, self.image_size,
                                                                        self.heatmap_size,
                                                                        self.num_joints, prev_2_optical_image,
                                                                        use_different_joints_weight=self.use_different_joints_weight,
                                                                        joints_weight=self.joints_weight)
            next_2_target_motion_heatmaps, target_heatmaps_weight = generate_motion_heatmaps(joints, joints_vis, self.sigma, self.image_size,
                                                                        self.heatmap_size,
                                                                        self.num_joints, next_2_optical_image,
                                                                        use_different_joints_weight=self.use_different_joints_weight,
                                                                        joints_weight=self.joints_weight)
            ...
            target_heatmaps, target_heatmaps_weight = generate_heatmaps(joints, joints_vis, self.sigma, self.image_size, self.heatmap_size,
                                                                        ...
                                                                        use_different_joints_weight=self.use_different_joints_weight,
                                                                        joints_weight=self.joints_weight)
            prev_target_motion_heatmaps = []
            next_target_motion_heatmaps = []
            prev_2_target_motion_heatmaps = []
            next_2_target_motion_heatmaps = []
        ...
        if self.train:
            return input_x, input_prev, input_next, target_heatmaps, target_heatmaps_weight, meta,\
                   input_prev_2, input_next_2, prev_target_motion_heatmaps, next_target_motion_heatmaps, \
                   prev_2_target_motion_heatmaps, next_2_target_motion_heatmaps
        else:
