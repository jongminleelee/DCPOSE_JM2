...
def generate_motion_heatmaps(joints, joints_vis, sigma, image_size, heatmap_size, num_joints, optical_image, **kwargs):
    """
        :param joints:  [num_joints, 3]
        :param joints_vis: [num_joints, 3]
        :param sigma:
    ...
    for joint_id in range(num_joints):
        ...
        if optical_image is None:
            ...
            optical_direction = (hsv_image[image_y, image_x, 0] + 0.94595945 * (hsv_image[image_y - 1, image_x, 0]
                                                                                + hsv_image[image_y, image_x - 1, 0] +
                                                                                hsv_image[image_y + 1, image_x, 0] +
                                                                                hsv_image[image_y, image_x + 1, 0])
                                 + 0.8948393 * (hsv_image[image_y - 1, image_x - 1, 0] + hsv_image[
                        image_y + 1, image_x + 1, 0]
                                                + hsv_image[image_y + 1, image_x - 1, 0] + hsv_image[
                                                    image_y - 1, image_x + 1, 0])) / 9
            ...
            optical_mag = (hsv_image[image_y, image_x, 1] + 0.94595945 * (hsv_image[image_y - 1, image_x, 1]
                                                                          + hsv_image[image_y, image_x - 1, 1] +
                                                                          hsv_image[image_y + 1, image_x, 1] +
                                                                          hsv_image[image_y, image_x + 1, 1])
                           + 0.8948393 * (
                                       hsv_image[image_y - 1, image_x - 1, 1] + hsv_image[image_y + 1, image_x + 1, 1]
                                       + hsv_image[image_y + 1, image_x - 1, 1] + hsv_image[
                                           image_y - 1, image_x + 1, 1])) / 9
            ...
            if (optical_image is None):
                ...
                y = x[:, np.newaxis]
                x0 = y0 = size // 2
                g = np.exp(- ((x - x0) ** 2 + (y - y0) ** 2) / (2 * (sigma-1) ** 2))

            else:
                g1 = np.exp(- ((x - x0 + 1 * math.cos(math.pi * optical_direction / 180)) ** 2 + (
                        y - y0 - 1 * math.sin(math.pi * optical_direction / 180)) ** 2) / (2 * sigma ** 2)) * 2 / 5

                x = np.arange(0, size, 1, np.float32)
                y = x[:, np.newaxis]
                g2 = np.exp(- ((x - x0 - (2 + optical_mag / 300) * math.cos(math.pi * optical_direction / 180)) ** 2 + (
                            y - y0 + (2 + optical_mag / 300) * math.sin(math.pi * optical_direction / 180)) ** 2) / (
                                        2 * sigma ** 2)) * 3 / 10

                x = np.arange(0, size, 1, np.float32)
                y = x[:, np.newaxis]
                g3 = np.exp(- ((x - x0 + (4 + 2 * optical_mag / 300) * math.cos(
                    math.pi * optical_direction / 180)) ** 2 + (y - y0 - (4 + 2 * optical_mag / 300) * math.sin(
                    math.pi * optical_direction / 180)) ** 2) / (2 * sigma ** 2)) * 3 / 10
